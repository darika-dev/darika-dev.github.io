<template>
  <div class="fixed inset-0">
    <div class="w-full h-full" id="example1"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateScene() {
      this.renderScene(),
      mesh.rotation.y = m(mesh.rotation.y, this.mouse.x, .075),
      mesh.rotation.x = m(mesh.rotation.x, this.mouse.y, .075),
      materialSphere.uniforms.uTime.value = this.clock.getElapsedTime(),
    ...
    startScene() {
      ...
      // Camera
      camera = new THREE.PerspectiveCamera(75,this.viewport.width / this.viewport.height,.1,10);
      ...
      mesh = new THREE.Mesh(sphere,materialSphere);
      scene.add(mesh);
    },
    ...
    onMouseMove(e) {
      let mouseX = this.mouse.x
        , mouseY = this.mouse.y;
      this.mouse.x = m(mouseX, e.clientX / this.viewport.width - .5, 1),
      this.mouse.y = m(mouseY, e.clientY / this.viewport.height - .5, 1)
    }
  },
};
</script>
